...
public set_fastknife(id)
{
    ...
    if(iWaitNext[id][seconds] > iTimestamp)
    {
        ...
        {
            client_print(id, print_chat, "%s: wait %d for your next use!", PLUGIN, iWaitNext[id][seconds] - iTimestamp)
        }
...
public Ham_SpawnPlayer_Post(id)
{
    ...
    if(get_pcvar_num(g_pActive) && !iWaitNext[id][fspawn])
    {
        iWaitNext[id][fspawn] = 1
        
        if(get_pcvar_num(g_pShowSprintMessage))
        {
